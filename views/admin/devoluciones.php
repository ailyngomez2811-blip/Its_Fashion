...
<head>
    ...
    <title>Its Fashion | Devoluciones</title>
...
<body class="bg-brand-light text-brand-dark antialiased">
    ...
    <aside class="w-64 h-screen fixed top-0 left-0 bg-brand-dark text-white flex flex-col z-50">
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1">
            ...
            <a href="devoluciones.php" class="sidebar-item active flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-white">
                <i class="fas fa-undo-alt w-5 text-center"></i> Devoluciones
                <?php if ($pendientes > 0): ?>
                    <span class="ml-auto bg-amber-400 text-white text-xs font-bold px-2 py-0.5 rounded-full"><?= $pendientes ?></span>
                <?php endif; ?>
            </a>
    ...
    <div class="ml-64 flex flex-col min-h-screen">
        <header class="glass-header sticky top-0 z-40 flex items-center justify-between px-8 h-20">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-2xl font-serif font-bold text-brand-dark">Solicitudes de devolución</h1>
            </div>
        </header>
            ...
            <div class="grid grid-cols-3 gap-4 mb-6">
                <div class="bg-white rounded-2xl p-5 border border-amber-100 stat-card">
                    <div class="flex items-center gap-3">
                        <div class="w-11 h-11 bg-amber-400 rounded-xl flex items-center justify-center"><i class="fas fa-clock text-white"></i></div>
                        <div>
                            <p class="text-2xl font-bold text-slate-800"><?= $pendientes ?></p>
                ...
                <div class="bg-white rounded-2xl p-5 border border-blue-100 stat-card">
                    <div class="flex items-center gap-3">
                        <div class="w-11 h-11 bg-brand-accent rounded-xl flex items-center justify-center"><i class="fas fa-undo-alt text-white"></i></div>
                        <div>
                            <p class="text-2xl font-bold text-slate-800"><?= $kpi['total'] ?? 0 ?></p>
                            <p class="text-xs text-slate-500">Aceptadas</p>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-2xl p-5 border border-red-100 stat-card">
                    <div class="flex items-center gap-3">
                        <div class="w-11 h-11 bg-red-500 rounded-xl flex items-center justify-center"><i class="fas fa-dollar-sign text-white"></i></div>
                        <div>
                            <p class="text-2xl font-bold text-slate-800">$<?= number_format($kpi['monto'] ?? 0, 0, ',', '.') ?></p>
                            <p class="text-xs text-slate-500">Monto devuelto</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden" style="box-shadow:0 2px 16px rgba(0,0,0,.04);">
                <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        <button onclick="filtrar('todos')" id="btn-todos" class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 transition">Todos</button>
                        <button onclick="filtrar('Pendiente')" id="btn-Pendiente" class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-amber-100 text-amber-700 hover:bg-amber-200 transition">Pendientes</button>
                        <button onclick="filtrar('Aceptada')" id="btn-Aceptada" class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-emerald-100 text-emerald-700 hover:bg-emerald-200 transition">Aceptadas</button>
                        <button onclick="filtrar('Rechazada')" id="btn-Rechazada" class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-red-100 text-red-600 hover:bg-red-200 transition">Rechazadas</button>
                    </div>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead>
                            <tr>
                                <?php foreach (['#', 'Fecha', 'Venta', 'Cliente', 'Motivo', 'Total', 'Estado', 'Acciones'] as $h): ?>
                            ...
                            <?php if (empty($devs)): ?>
                                <tr>
                                    <td colspan="8" class="px-6 py-16 text-center text-slate-400">
                                        ...
                                        <p class="text-sm">No hay solicitudes de devolución</p>
                                    </td>
                                </tr>
                            <?php else: ?>
                                ...
                                ?>
                                    <tr class="trow" data-estado="<?= $d['estado'] ?>">
                                        <td class="px-5 py-4 border-b border-slate-50 text-sm font-mono text-slate-500">#<?= $d['id_devolucion'] ?></td>
                                        <td class="px-5 py-4 border-b border-slate-50 text-sm text-slate-700"><?= date('d/m/Y H:i', strtotime($d['fecha'])) ?></td>
                                        <td class="px-5 py-4 border-b border-slate-50 text-sm font-mono text-blue-600">#<?= $d['id_venta'] ?></td>
                                        <td class="px-5 py-4 border-b border-slate-50 text-sm text-slate-700"><?= htmlspecialchars($d['cliente_nombre'] ?? 'Mostrador') ?></td>
                                        <td class="px-5 py-4 border-b border-slate-50 text-sm text-slate-500 max-w-xs truncate"><?= htmlspecialchars($d['motivo']) ?></td>
                                        <td class="px-5 py-4 border-b border-slate-50 text-sm font-bold text-red-600">$<?= number_format($d['total_devolucion'], 2) ?></td>
                                        ...
                                        <td class="px-5 py-4 border-b border-slate-50">
                                            <div class="flex items-center gap-2">
                                                <button onclick="verDetalle(<?= $d['id_devolucion'] ?>)" title="Ver detalle"
                                                    class="w-8 h-8 rounded-lg bg-slate-100 hover:bg-blue-100 text-slate-500 hover:text-blue-600 flex items-center justify-center transition">
                                                    <i class="fas fa-eye text-xs"></i>
                                                </button>
                                                <?php if ($d['estado'] === 'Pendiente'): ?>
                                                    <button onclick="resolver(<?= $d['id_devolucion'] ?>, 'aprobar')" title="Aceptar"
                                                        class="w-8 h-8 rounded-lg bg-emerald-100 hover:bg-emerald-200 text-emerald-600 flex items-center justify-center transition">
                                                        <i class="fas fa-check text-xs"></i>
                                                    </button>
                                                    <button onclick="resolver(<?= $d['id_devolucion'] ?>, 'rechazar')" title="Rechazar"
                                                        class="w-8 h-8 rounded-lg bg-red-100 hover:bg-red-200 text-red-500 flex items-center justify-center transition">
    ...
    <script>
        const CTRL = '../../controllers/admin/DevolucionController.php';

        function filtrar(estado) {
            document.querySelectorAll('#tabla-body tr[data-estado]').forEach(tr => {
                tr.style.display = (estado === 'todos' || tr.dataset.estado === estado) ? '' : 'none';
            });
        }

        function resolver(id, accion) {
            const label = accion === 'aprobar' ? 'aceptar' : 'rechazar';
            if (!confirm(`¿Seguro que deseas ${label} esta solicitud?`)) return;
        ...
        function verDetalle(id) {
            fetch(`${CTRL}?action=detalle&id=${id}`)
                .then(r => r.json())
                .then(items => {
                    const body = document.getElementById('det-body');
                    if (!items.length) {
                        body.innerHTML = '<p class="text-slate-400 text-sm text-center py-8">Sin detalle</p>';
                    } else {
                    ...
                    <thead><tr>
                        ...
                        <th class="text-right py-2 text-xs text-slate-500 uppercase">Total</th>
                    </tr></thead><tbody>` +
                        ...
                        <td class="py-3 text-right font-bold text-red-600">$${(i.cantidad * parseFloat(i.precio_unitario)).toFixed(2)}</td>
                    </tr>`).join('') + '</tbody></table>';
                    }
